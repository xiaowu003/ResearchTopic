...
module cohesive_law_mod                                                                                                                                                                                                                                         
    ...
    subroutine calculate_mixed_mode_rate(tdelta,ndelta,beita)                                                                                                                                                                                       
        ...
        real(kind=RK),intent(out)::beita                                                                                                                                                                                                                            
         ...
         return                                                                                                                                                                                                                                                      
    end subroutine calculate_mixed_mode_rate
end module                                                                                                                                                                                                                                                      
  
    subroutine Constitutive( DMICRT,delta10,delta20,uRelaLocal,Gnc, Gtc,exp,delta2f,delta1f,rkn,OldSDEG,SDEG,tau,strength)    
        include 'vaba_param.inc'
        intent(inout):: DMICRT
        ...
        if( DMICRT < 1.0d0 )then                               
            DMICRT=( dim(tau(1),0.0d0)/strength(1) )**2 + ( tau(2)/strength(2) )**2
          
            if (DMICRT>=1.0d0) then  
                DMICRT=2.0d0 
                call CalculateSDEG(delta10,delta20,uRelaLocal,Gnc,Gtc,exp,delta2f,delta1f,rkn,OldSDEG,SDEG)     
            else                        
                SDEG=0.0d0   
            end if
    ...
    subroutine CalculateSDEG(delta10,delta20,uRelaLocal,Gnc,Gtc,exp,delta2f,delta1f,rkn,OldSDEG,SDEG)
        include 'vaba_param.inc'
        intent(out):: SDEG
        intent(in):: OldSDEG
        intent(in):: delta10,delta20,Gnc,Gtc
        ...
        if ( uRelaLocal(1)>0.0d0 ) then                            
            ...
            deltaM0 = delta10*delta20*sqrt((1+ratioMix**2)/(delta20**2 + (ratioMix*delta10)**2))
            write(6,*) 'deltaM0',deltaM0
            deltaMf = 2/rkn/deltaM0 * (Gnc+(Gtc-Gnc)*(ratioMix**2 / (1+ratioMix**2)) ** exp)
            write(6,*) 'deltaMf',deltaMf
        else 
            deltaM0 = delta20
            deltaMf = delta2f
        end if
            SDEG_NEW =(deltaMf*(deltaM-deltaM0)) / ((deltaMf-deltaM0)*deltaM)
            SDEG  = max(OldSDEG ,SDEG_NEW ) 
        write(6,*) '打印输出'
        write(6,*) 'SDEG',SDEG
        return
    end subroutine CalculateSDEG
    ...
    subroutine mass_matrix(nblock,rho,coords,ndofel,amass)          
        ...
        do kblock = 1, nblock
            EleCoordVector(1,:)=coords(kblock,2,:)-coords(kblock,1,:)    !节点2与节点1的xyz三坐标差值，
                                                                         !即节点2的以节点1为原点的向量表示
            ...
            call vector_cross(EleCoordVector(1,:),EleCoordVector(2,:),V1)!v1为节点2和3的外积
  !#TAG 这里计算三角形面积的方法可以用到
            S1= half*vector_mod( V1 )                                    !利用外积计算三角形面积
            call vector_cross(EleCoordVector(2,:),EleCoordVector(3,:),V2)                                                          !v2为节点3和4的外积
  ...
  ! ============================ 将材料属性等值传入 ================================
    subroutine key_parameter(props,constant,rkn,rkt,strenthN,strenthT,strength,Gnc,Gtc, &
        rho,exp,rknt,delta10,delta20,delta1f,delta2f,gaussPara)  !! 输入需要的参数
        ...
        !gaussPara是一个两行四列的数组   !数组切片并赋值，表示gausspara的前两行，第一列，分别赋值后面两个
  ...
  !====================== 计算中性面上点的坐标和节点分离量 =========================
    subroutine seperation_and_middle_suface(u,coords,disNode,   &
        ...
        iBgn = 1
            ...
            do k = 1, nGP
               xyzMidsurf(:,k) = ( coords(kblock,k,:) + coords(kblock,k+nGP,:) + disNode(:,k)+disNode(:,k+nGP)) / two   ! 中性面上的节点坐标
  ...
  !==================计算切法向分离量和切法向单位向量============================   
    subroutine normal_seperation_and_shear_seperation_and_unitVect (drds,drdt,xyzMidsurf,rShape,  &
        uRelative,vNormalMod,vNormal,shearUnit,uRelaLocal)
        ...
        tangs = drds(1)*xyzMidsurf(:,1) + drds(2)*xyzMidsurf(:,2) + drds(3)*xyzMidsurf(:,3) + drds(4)*xyzMidsurf(:,4)
        tangt = drdt(1)*xyzMidsurf(:,1) + drdt(2)*xyzMidsurf(:,2) + drdt(3)*xyzMidsurf(:,3) + drdt(4)*xyzMidsurf(:,4)
        ...
        uRelaGP(:) = uRelative(:,1)*rShape(1) + uRelative(:,2)*rShape(2) + uRelative(:,3)*rShape(3) + uRelative(:,4)*rShape(4)   
        ...
        return
    end subroutine normal_seperation_and_shear_seperation_and_unitVect
 ...
 !================================================VUEL子程序=========================================================
    subroutine vuel(nblock,                                     &
        ...
        use math
        include 'vaba_param.inc'   
        parameter (zero = 0.d0, half = 0.5d0, one = 1.d0, two=2.d0, three=3.d0, quarder=0.25d0, eighth=0.125d0)
        ...
        parameter ( jDynExplicit = 17              &    !这里表示非线性的直接积分的显示动态过程分析
                    jCouplingExplict = 74)              !这里表示热力耦合分析
        ...
        if (jtype .eq. 2 .and. lflags(iProcedure).eq.jDynExplicit) then
            !! 参数赋值
            call key_parameter(props,constant,rkn,rkt,strenthN,                 &
            ...
            if ( lflags(iOpCode).eq.jMassCalc ) then  
                ...
                do kblock = 1, nblock
                    !! 计算中性面点的坐标以及每对节点的分开量
                    call seperation_and_middle_suface(u,coords,disNode,nGP,     &
                    ...
                    do iGp=1,nGP
                        ...
                        tau(1)=svars(kblock,iNumber+2)                ! tao τ express shear stress
                        tau(2) =svars(kblock,iNumber+3)
                        
                        !! 求形函数及其偏导
                        call shape_q4(gaussPara(1,iGp), gaussPara(2,iGp),       &
                                      rShape, drds, drdt)

                        !! 求单位法向量和单位切向量，切法向分离量 l
