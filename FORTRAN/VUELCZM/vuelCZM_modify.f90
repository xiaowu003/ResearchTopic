...
module parameter_mod
    ...
    integer, save :: nCoh                                       !! 一次进来的内聚力单元数量 =nblock
    integer, save :: nNdCoh                                     !! 一个内聚力单元节点数 8=nnode
    integer, save :: nHalfNd                                    !!一个内聚力单元一半节点数 4
    integer, save :: ndofnode                                   !! 节点自由度数 3 or 4 = ndofel / nNode

    !integer, save :: nDim = 3                                  !!自由度数 3
    !integer, save :: nDofCoh = 24                              !!一个内聚力单元总自由度数 24

    integer, save :: nDim                                       !! 节点的坐标数量 3维就是三个坐标
    integer, save :: nDofCoh                                    !!一个内聚力单元总自由度数 32=ndofel
    ...
    real(kind=RK), save :: m_deltaNo                            !! 拉伸状态下开始损伤的分离量
    real(kind=RK), save :: m_deltaSo                            !! 剪切状态下开始损伤的分离量
    real(kind=RK), save :: m_Nstiff                             !! 法向罚刚度
    real(kind=RK), save :: m_Sstiff                             !! 切向罚刚度
    ...
    real(kind=RK), save :: h_solid                              !!固体导热系数（暂定）#bug
    real(kind=RK), save :: h_gas                                !!空气导热系数（暂定）
    real(kind=RK), save :: h_contact                            !!接触导热系数（暂定）  
...
module cohesive_law_mod
    ...
    subroutine compute_input_coh_displacement(stiff,nStreng,tStreng,GN,GS,alfa,deltaNo,deltaSo,tdelta,ndelta,deltai,deltaf) !! 计算单元开始损伤时的分离量和完全损伤时的分离量
        ...
        real(kind=RK),intent(in) :: deltaNo  !!拉伸状态下开始损伤的分离量 =m_deltaNo
        real(kind=RK),intent(in) :: deltaSo  !!剪切状态下开始损伤的分离量 =m_deltaSo
    ...
    subroutine cohesive_stress_at_one_point(Nstiff,Sstiff,deltai,deltaf,delta,det_ndelta,det_tdelta,eff_delta,ghis,nvect,tvect,fail,sig,damage)
        !该子程序的参数解释为GPT生成，可能有错误，请参考
        implicit none
        real(kind=RK),intent(in):: Nstiff           !! 法向刚度
        real(kind=RK),intent(in):: Sstiff           !! 切向刚度
        ...
        real(kind=RK):: coef=0.0d0                      !!损伤变量D，这里为什么将它初始化为0？没有确保该值始终不会变小,引入新的损伤变量damage来记录所有的损伤变量值
        real(kind=RK),intent(inout) :: damage       !!传入的旧损伤变量和新计算的损伤变量比较进行更新

        write(*,*) "enter cohesive_stress_at_one_point"
        ...
        if(ghis < 0.0d0) then
            write(*,*) "here ghis < 0.0d0, return"
            goto 111     !#bug 这里也是 实际分离量是否可以为 0 呢？ 暂时改为可以为 0
        end if
        ...
        if(ghis <= deltai) then
            ...
            if(det_ndelta > 0.0d0) then
                !nsig = stiff * det_ndelta * nvect    ! WRONG 这里两个实数相乘得到了数组，错误, 再乘单位法向量也错，这里需要的是发向应力值而不是数组
                dnsig = Nstiff * det_ndelta           ! RIGHT!
            end if

            dtsig = Sstiff * det_tdelta    !#bug 这里的刚度和计算法向应力的shift刚度一样吗？
            ...
            if(det_ndelta > 0.0d0) then
                dnsig = (1.0d0 - coef) * Nstiff * det_ndelta   ! #bug 这里从1改成了1.0d0
            end if

            dtsig = (1.0d0 - coef) * Sstiff * det_tdelta
        end if
        !else
        !goto 555
        !end if
        ...
        if(modu <= 0.0d0) then !#bug 这里一样是否可以 =0 ? 暂时改为可以为 0,改为不能为0，为0时下方计算单位法向量会有bug
            write(*,*) "here modu < 0.0d0, terminate"
            goto 222
        end if

        bvect = delta
        bvect = bvect/modu  !!得到上下两节点间的单位向量 #bug 这里modu如果为0，会出bug，bvect会变成NaN
        rnvect = nvect
        rtvect = tvect
...
module cohesive_element_mod
    ...
    implicit none
    private
    public :: mass_matrix, set_parameter, one_element_cohesive_force, calculate_node_heat_flow
    character(len=MNL) :: msg
    contains

    subroutine set_parameter(nBlock,nNode,mcrd,ndofel,nProps,props) !!参数传递
        implicit none
        ...
        m_deltaNo = m_nStrength / m_Nstiff
        !m_deltaSo = m_tStrength/m_Nstiff      !#bug 这里为什么还是stiff？
        m_deltaSo = m_tStrength / m_Sstiff       !暂时改为切向罚刚度
    ...
    subroutine mass_matrix(nBlock,nNode,mcrd,ndofel,coords,amass)
        ...
        do i = 1, nCoh
            ...
            do j=1,nDofCoh
                ! When the mass matrix is calculated, lflags(iProcedure) = 17, that is, there are only 24 degrees of freedom are involved, and
    ...
    subroutine one_element_cohesive_force(iEleM,nVar,u,coords,var,rhs_elem)     !!计算一个内聚力单元的 force 这里的force是指力 N
        implicit none
        integer, intent(in) :: iElem                    !!iElem是  第几个element
        ...
        real(kind=RK) :: fcNd(ndofnode,nNdCoh)  !! =rhs array #bug (4,8) may be not allowed, value range error， 这里传入的是(1,32),但是定义成(4,8)
        real(kind=RK),intent(out) :: rhs_elem(nDofCoh)   !! =rhs(i,:) array
        ...
        real(kind=RK) :: gapNd(3,4)     !! The vector from the node below the cell to the node above it #bug 
        !real(kind=RK) :: fcAdd(4,4)
        real(kind=RK) :: fcAdd(ndofnode,nHalfNd)    !!自适应大小
        real(kind=RK) :: fcPt(3)         !!计算传出的 力向量
        real(kind=RK) :: damage_old      !!新计算的损伤变量, 这里的单元损伤变量是整个单元的吗，还是每个节点都有一个损伤变量值； 是每个积分点都有一个损伤变量
        real(kind=RK) :: nGap            !!法向分离量，计算导热系数时会用到
        ...
        do i=1,nInteg2D
            ...
            call one_point_cohesive_force(var,nvar,det,fCount,maxEffGap,gapPt,normal,fail,fcPt,damage_old,nGap)
            ...
            call calculate_node_heat_flow(u,damage_old,nGap,fcAdd)
            
            fcNd(:,1:nHalfNd) = fcNd(:,1:nHalfNd) + fcAdd !!#bug这里fnd和fcadd的数组大小可能不匹配
            ibgn = ibgn + 3
        end do
        ...
        !fcNd = reshape( rhs_elem, (/ndofnode,nNdCoh/) )
        rhs_elem = reshape( fcNd, (/nDofCoh/)) !将（4,8）的二维数组转化为长为32的一维数组，并传出
    ...
    subroutine distribute_force_to_nodes(xi,fcPt,fcNd)
        ...
        real(kind=RK),intent(out)::fcNd(ndofnode,nHalfNd)
        real(kind=RK)::shap(4)
        integer::nNum, i
        ...
        nNum = nInteg2D
        do i=1,nNum
            fcNd(1:3,i) = shap(i) * fcPt
        end do
        return
    end subroutine distribute_force_to_nodes

    subroutine calculate_node_heat_flow(u,damage_old,nGap,fcAdd)
        implicit none
        real(kind=RK),intent(in) :: u(nDofCoh)
        real(kind=RK),intent(in) :: damage_old
        real(kind=RK),intent(in) :: nGap
        real(kind=RK),intent(inout) :: fcAdd(ndofnode,nHalfNd)
        real(kind=RK):: delt_t(4)
        !real(kind=RK):: qcz(4)
        real(kind=RK):: hcz
        integer :: i
        integer :: num_t_bottom, num_t_top
        ...
        if(ndofnode < 4) then
            ...
            if(nGap >= 0.0d0) then
                hcz = h_solid * (1.0d0 - damage_old) + h_gas * damage_old * nGap
            else
                hcz = h_solid * (1.0d0 - damage_old) - h_contact * damage_old * nGap
            end if

            do i = 1, nInteg2D
                num_t_top = num_t_bottom + 16

                delt_t(i) = u(num_t_top) - u(num_t_bottom)

                fcAdd(4,i) = fcAdd(4,i) + delt_t(i) * hcz

                num_t_bottom = num_t_bottom + 4
            end do
        end if
        return
    end subroutine calculate_node_heat_flow
    ...
    subroutine seperation_and_middle_surface(u,coords,gap,xyzMidsurf)     !!计算中间接截面节点坐标，以及上下节点间距向量
        implicit none
        real(kind=RK), intent(in) :: u(nDofCoh)             !!u(24) or u(32)
        real(kind=RK), intent(in) :: coords(nNdCoh,nDim)    !!coords(8,3)
        real(kind=RK), intent(out) :: gap(nDim,nInteg2D)    !!(3,4)二维数组， 由下方节点指向上方节点的向量
        real(kind=RK), intent(out) :: xyzMidsurf(3,4)       !!6面体单元变形后上下对应两节点之间 中间点 的坐标
        real(kind=RK) :: disNode(ndofnode,nNdCoh)               !!(3,8)二维数组
        integer :: k

        disNode = reshape( u, (/ndofnode,nNdCoh/) )  ! ndofnode = 3 or 4, nNdCoh = 8, (3,8) or (4,8)
        !iBgn = 1
        !do k=1,nnode
        ...
        do k = 1, nHalfNd  ! Here the value of k ranges from 1 to 4, but coords
            ...
            xyzMidsurf(:,k) = ( coords(k,:)+coords(k + nInteg2D,:)+ disNode(1:3,k)+disNode(1:3,k+nInteg2D))/2.0d0  !原来相对应两个节点的坐标值加位移量 再 除以2，得到两节点中间点坐标
           
            gap(:,k) = disNode(1:3,k+nInteg2D) + coords(k + nInteg2D,:) - coords(k,:) - disNode(1:3,k)  !#bug 上下对应节点坐标值相减，为上下节点间距向量，为下方节点指向上方节点的向量, 而对应的上下节点的初始坐标应该是相等的，所以加不加都可以
        
        end do
        return
    end subroutine seperation_and_middle_surface

    subroutine one_point_cohesive_force(var,nvar,area,fCount,ghis,gapVect,normal,fail,fc,damage,nGap)  !! gpos, Hpos, fcount
        implicit none
        ...
        !integer,intent(inout)::fcount
        real(kind=RK) :: tmod
        real(kind=RK),intent(out) :: nGap        !! 下方节点指向上方节点的向量 与 积分点单位法向量 的 点乘，所以可能会出现 负值
        ...
        write(*,*) "calculate one_point_cohesive_force"
        ...
        if(nGap < 0.0) then               !单位法向量与下节点指向上节点的向量方向不同，角度超过90度， 发生了穿透所以用 罚函数法 限制穿透
            tfc = -abs(nGap) * m_Nstiff * area * normal  ! 这里计算出来的是什么？ --罚函数法 得到的法向力向量
            fc  = fc + tfc
        end if

        call calculate_effective_displacement_point(nGap,tGap,effDelta)  !计算实际总分离量，不是法向或切向，是合成的总分离量
        if(effDelta <= 0.0d0) then   !#bug 这里是否可以等于 0? 暂时改为可以为 0
            write(*,*) "one_point_cohesive_force : effDelta = "
            write(*,*) effDelta
            return
        end if

        call compute_input_coh_displacement(m_Nstiff,m_nStrength,m_tStrength,m_Gnc,m_Gtc,m_alpha,m_deltaNo,m_deltaSo,tGap,nGap,deltai,deltaf)
        ...
        if(effDelta >= deltaf) then !failure of the gauss point.
            ...
            call cohesive_stress_at_one_point(m_Nstiff,m_Sstiff,deltai,deltaf,gVect,nGap,tGap,effDelta,ghis,normal,tvect,fail,co_sig,damage)
            tfc = co_sig * area
            fc = fc + tfc
        end if
        return
...
!================================================VUEL=========================================================
    subroutine vuel(nblock,rhs,amass,dtimeStable,svars,nsvars,energy,nnode,ndofel,props,nprops,jprops,njprops,coords,mcrd,u,du,v,a,jtype,jElem,time,period,dtimeCur,dtimePrev,kstep,kinc,lflags,dMassScaleFactor,predef,npredef,jdltyp,adlmag)
...
!       Debug
        ...
        integer :: i
        !real(kind=RK) :: array1(32)
        !real(kind=RK) :: array2(4,8)

        !write(*,*) "lflags(iProcedure) = "
        !write(*,*) lflags(iProcedure)
        ...
        !if( jType==2 .and. lflags(iProcedure)==jDynExplicit ) then          !直接积分的显示动力学分析
            ...
            if( lflags(iOpCode) == jMassCalc )then  
                ...
                do i = 1, nblock
                    ...
                    write(6,*) 'node force and temperature'
                    write(6,*) rhs(i,:)
                    !write(6,*) rhs(i,17:32)
